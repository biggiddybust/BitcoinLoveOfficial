...
CzBLVWallet::CzBLVWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLVSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLVSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLV seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBLVWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLVCount(nCountLastUsed);
    else if (!walletdb.ReadZBLVCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLVWallet::Lock()
{
    seedMaster = 0;
}

void CzBLVWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLVWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLV(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLVWallet::LoadMintPoolFromDB()
{
...
void CzBLVWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLVWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLVWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBLVWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLV(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBLVWallet::SeedToZBLV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBLVWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLVWallet::GenerateDeterministicZBLV(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLVWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLVWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
