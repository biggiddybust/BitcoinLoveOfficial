...
#ifndef BITCOINLOVE_ZBLVWALLET_H
#define BITCOINLOVE_ZBLVWALLET_H
...
public:
    CzBLVWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBLV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBLV(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BITCOINLOVE_ZBLVWALLET_H
