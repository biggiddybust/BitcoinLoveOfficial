...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BLV);
    ...
    if (!settings.contains("nAnonymizeBitcoinLoveAmount"))
        settings.setValue("nAnonymizeBitcoinLoveAmount", 1000);

    nAnonymizeBitcoinLoveAmount = settings.value("nAnonymizeBitcoinLoveAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitcoinLoveAmount"))
        SoftSetArg("-anonymizebitcoinloveamount", settings.value("nAnonymizeBitcoinLoveAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcoinlove.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinLoveAmount:
            return QVariant(nAnonymizeBitcoinLoveAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinLoveAmount:
            nAnonymizeBitcoinLoveAmount = value.toInt();
            settings.setValue("nAnonymizeBitcoinLoveAmount", nAnonymizeBitcoinLoveAmount);
            emit anonymizeBitcoinLoveAmountChanged(nAnonymizeBitcoinLoveAmount);
            break;
        case CoinControlFeatures:
