...
// Copyright (c) 2017-2019 The BitcoinLove Developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BLV)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sBLVPercentage, QString& szBLVPercentage)
{
    int nPrecision = 2;
    ...
    szBLVPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sBLVPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    // ui->labelBLVPercent->setText(sPercentage);
    // ui->labelzBLVPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zBLV.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in bitcoinlove.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in bitcoinlove.conf");
    }
    ...
    bool showBLVAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyBLVAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showBLVPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBLVPending = watchUnconfBalance != 0;
    bool showBLVLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBLVLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showBLVAvailable || showWatchOnlyBLVAvailable);
    ui->labelBalanceText->setVisible(showBLVAvailable || showWatchOnlyBLVAvailable);
    ui->labelWatchAvailable->setVisible(showBLVAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showBLVPending || showWatchOnlyBLVPending);
    ui->labelPendingText->setVisible(showBLVPending || showWatchOnlyBLVPending);
    ui->labelWatchPending->setVisible(showBLVPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showBLVLocked || showWatchOnlyBLVLocked);
    ui->labelLockedBalanceText->setVisible(showBLVLocked || showWatchOnlyBLVLocked);
    ui->labelWatchLocked->setVisible(showBLVLocked && showWatchOnly);
    ...
    bool showzBLVAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzBLVUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzBLVImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    // ui->labelzBalanceMature->setVisible(showzBLVAvailable);
    // ui->labelzBalanceMatureText->setVisible(showzBLVAvailable);
    // ui->labelzBalanceUnconfirmed->setVisible(showzBLVUnconfirmed);
    // ui->labelzBalanceUnconfirmedText->setVisible(showzBLVUnconfirmed);
    // ui->labelzBalanceImmature->setVisible(showzBLVImmature);
    // ui->labelzBalanceImmatureText->setVisible(showzBLVImmature);
    //bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    //ui->labelBLVPercent->setVisible(showPercentages);
    // ui->labelzBLVPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BLV")
    updateDisplayUnit();
}
