...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BLV at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBLV send
enum ZerocoinSpendStatus {
    ZBLV_SPEND_OKAY = 0,                            // No error
    ZBLV_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLV_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLV_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLV_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLV_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLV_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLV_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLV_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLV_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBLV_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLV_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLV_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLV_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLV_SPENT_USED_ZBLV = 14,                      // Coin has already been spend
    ZBLV_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZBLV_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZBLVOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBLVWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBLVTracker> zphrTracker;
    ...
    void setZWallet(CzBLVWallet* zwallet)
    {
        zwalletMain = zwallet;
        zphrTracker = std::unique_ptr<CzBLVTracker>(new CzBLVTracker(strWalletFile));
    }

    CzBLVWallet* getZWallet() { return zwalletMain; }
