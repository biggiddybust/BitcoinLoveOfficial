...
#include "config/bitcoinlove-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BLV and zBLV money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 5771, 11773));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BLV/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BLV/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bitcoinlove, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BLV/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BitcoinLove specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:5771"));
    ...
    strUsage += HelpMessageOpt("-zphrbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBLV backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizebitcoinloveamount=<n>     " + strprintf(_("Keep N BLV anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 5772, 11774));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The BitcoinLove Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that BitcoinLove is running in a usable environment with all
...
#endif
    // Make sure only a single BitcoinLove process is using the data directory.
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BitcoinLove specific: zerocoin and spork DB's
                ...
                // BitcoinLove: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBLVMinted();
                        RecalculateZBLVSpent();
                    }
                    RecalculateBLVSupply(1);
                }
                ...
                // BitcoinLove: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzBLVWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zBLV wallet..."));
...
//    }
//
//    nAnonymizeBitcoinLoveAmount = GetArg("-anonymizebitcoinloveamount", 0);
//    if (nAnonymizeBitcoinLoveAmount > 999999) nAnonymizeBitcoinLoveAmount = 999999;
//    if (nAnonymizeBitcoinLoveAmount < 2) nAnonymizeBitcoinLoveAmount = 2;
    ...
    LogPrintf("Anonymize BitcoinLove Amount %d\n", nAnonymizeBitcoinLoveAmount);
    ...
    /* Denominations
       ...
       For example:
       1BLV+1000 == (.1BLV+100)*10
       10BLV+10000 == (1BLV+1000)*10
    */
