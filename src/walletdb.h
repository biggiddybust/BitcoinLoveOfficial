...
public:
    ...
    bool WriteZBLVSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBLVSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBLVSeed_deprecated(uint256& seed);
    bool EraseZBLVSeed();
    bool EraseZBLVSeed_deprecated();

    bool WriteZBLVCount(const uint32_t& nCount);
    bool ReadZBLVCount(uint32_t& nCount);
